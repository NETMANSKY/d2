...
function theme_stylize_pagination($pagination) {
	if ($pagination) {
		$pagination = '<div class="pager">' . str_replace(array('current', 'dots'), array('current active', 'dots more'), $pagination) . '</div>';		
	}
	return $pagination;
}
...
function theme_comment_reply_link_filter($link) {
	return str_replace('class=\'', 'class=\'button ', $link);
}
...
function theme_comment($comment, $args, $depth) {
	...
	switch ($comment->comment_type) :
		...
		<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
			<div class="comment <?php echo $comment->comment_type ?> clearfix" id="comment-<?php comment_ID() ?>">
    <div class="comment-avatar"><?php echo theme_get_avatar(array('id' => $comment, 'size' => 80)); ?></div>
    <div class="comment-inner">
        <div class="comment-header comment-meta commentmetadata"><?php printf(__('%s on ', THEME_NS), get_comment_author_link($comment->comment_ID)); ?>
...
<?php edit_comment_link(__('(Edit)', THEME_NS)); ?></div>
        <div class="comment-content comment-body"><?php if ($comment->comment_approved == '0') : ?>
    <em><?php _e('Your comment is awaiting moderation.', THEME_NS); ?></em>
    <br />
<?php endif; ?>
<?php comment_text(); ?></div>
        <div class="comment-footer reply"><?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => $args['max_depth']))); ?></div>
    </div>
</div>
			<?php
			break;
		...
		<li class="post pingback">
			<div class="comment <?php echo $comment->comment_type ?> clearfix">
				<div class="comment-content comment-body"><?php _e('Pingback:', THEME_NS); ?> <?php comment_author_link(); ?><?php edit_comment_link(__('(Edit)', THEME_NS), ' '); ?></div>
			</div>
			<?php
			break;
	endswitch;
